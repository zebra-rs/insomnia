...
const CONFIG_PATH: &[&str] = &["vpn", "ipsec"];

/// Backend task: a JSON subscription to `/vpn/ipsec` (snapshot first,
/// then one whole-subtree batch per touching commit) selected with
/// the show orders the provider routes here. Reconnects with backoff
/// when the stream drops; the fresh snapshot re-renders swanctl.conf,
/// which is idempotent.
pub async fn run(
    host: String,
    conf_dir: PathBuf,
    vici_socket: PathBuf,
    mut show_rx: mpsc::Receiver<ShowRequest>,
) {
    loop {
            ...
            tokio::select! {
                msg = stream.message() => match msg {
                    Ok(Some(event)) => {
                        ...
                        process(update, &conf_dir).await;
                    }
                    Ok(None) => break,
                    Err(err) => {
                        tracing::warn!("ipsec: subscription error: {err}");
                        break;
                    }
                },
                Some(req) = show_rx.recv() => process_show(&vici_socket, req).await,
            }
        }
        tracing::warn!("ipsec: subscription ended; reconnecting");
...
async fn process_show(vici_socket: &Path, req: ShowRequest) {
    let out = match req.path.as_str() {
...
";
    ...
    use crate::vici::test_encode::{key_value, list, section};
