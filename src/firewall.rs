...
const CONFIG_PATH: &[&str] = &["firewall"];
...
/// Backend task: a JSON subscription to `/firewall` (snapshot first,
/// then one whole-subtree batch per touching commit) selected with
/// the show orders the provider routes here. When the stream drops
/// the task reconnects with backoff and the fresh snapshot re-applies
/// the ruleset — idempotent, since every apply is one atomic replace.
pub async fn run(host: String, mut show_rx: mpsc::Receiver<ShowRequest>) {
            ...
            tokio::select! {
                msg = stream.message() => match msg {
                    Ok(Some(event)) => {
                        ...
                        process(update, &mut state).await;
                    }
                    Ok(None) => break,
                    Err(err) => {
                        tracing::warn!("firewall: subscription error: {err}");
                        break;
                    }
                },
                Some(req) = show_rx.recv() => process_show(&state, req).await,
            }
        }
        tracing::warn!("firewall: subscription ended; reconnecting");
...
async fn process_show(state: &ShowState, req: ShowRequest) {
