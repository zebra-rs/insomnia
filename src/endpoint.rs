//! zebra-rs vty gRPC endpoint dialing, copied from vtyctl's
//! `endpoint.rs` so both clients accept the same `--host` spellings.
...
/// Delay between reconnection attempts whenever a connection or a
/// stream toward zebra-rs fails.
pub const RECONNECT_DELAY: Duration = Duration::from_secs(3);

/// Like [`connect`], but retries forever: insomnia is a daemon and
/// zebra-rs being down (not yet started, restarting) is a normal
/// condition to ride out, not an error to die on.
pub async fn connect_retry(host: &str) -> Channel {
    let uri = host_uri(host);
    loop {
        match connect(&uri).await {
            Ok(channel) => return channel,
            Err(err) => {
                tracing::warn!("connect {uri}: {err}; retrying");
